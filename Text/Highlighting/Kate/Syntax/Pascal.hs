...
startingState = SyntaxState {synStContexts = fromList [("Pascal",["Normal"])], synStLanguage = "Pascal", synStCurrentLine = "", synStCharsParsedInLine = 0, synStCaseSensitive = True, synStKeywordCaseSensitive = False, synStKeywordDelims = " \n\t.():!+,-<=>%&*/;?[]^{|}~\\", synStCaptures = []}
...
parseRules "Normal" = 
  do (attr, result) <- (((pRegExpr (compileRegex "\\b(begin|case|record)(?=(\\{[^}]*(\\}|$)|\\(\\*.*(\\*\\)|$))*([\\s]|$|//))") >>= withAttribute "Keyword"))
                        ...
                        <|>
                        ((pKeyword ["and","array","asm","case","const","div","do","downto","else","file","for","function","goto","if","in","label","mod","nil","not","of","operator","or","packed","procedure","program","record","repeat","set","then","to","type","unit","until","uses","var","while","with","xor","at","automated","break","continue","dispinterface","dispose","exit","false","finalization","initialization","library","new","published","resourcestring","self","true"] >>= withAttribute "Keyword"))
                        <|>
                        ((pKeyword ["abstract","as","bindable","constructor","destructor","except","export","finally","import","implementation","inherited","inline","interface","is","module","on","only","otherwise","override","private","property","protected","public","read","qualified","raise","restricted","shl","shr","threadvar","try","virtual","write"] >>= withAttribute "ISO/Delphi Extended"))
                        <|>
                        ((pKeyword ["Integer","Cardinal","ShortInt","SmallInt","LongInt","Int64","Byte","Word","LongWord","Char","AnsiChar","WideChar","Boolean","ByteBool","WordBool","LongBool","Single","Double","Extended","Comp","Currency","Real","Real48","String","ShortString","AnsiString","WideString","Pointer","Variant","File","Text"] >>= withAttribute "Type"))
                        <|>
...
parseRules "Comment1" = 
  do (attr, result) <- (((pKeyword ["FIXME","TODO","###"] >>= withAttribute "Alert"))
                        <|>
